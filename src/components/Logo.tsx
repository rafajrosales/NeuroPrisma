...
export default function Logo({ className, size = 'md', showText = false }: LogoProps) {
  ...
  return (
    <div className={cn("flex items-center gap-2 md:gap-3", className)}>
      <div className={cn(
        "relative flex items-center justify-center rounded-xl md:rounded-2xl overflow-hidden shadow-lg",
        dimensions[size],
        ...
        <svg 
          ...
          className="text-white w-[65%] h-[65%] relative z-10"
        >
      ...
      {showText && (
        <div className="flex flex-col">
          <span className={cn(
            "font-black tracking-tighter text-text-main leading-none",
            size === 'xs' ? 'text-sm' : size === 'sm' ? 'text-lg' : size === 'md' ? 'text-xl' : 'text-3xl'
          )}>
            NeuroPrisma
          </span>
          {size !== 'xs' && (
